...
Aunque los valores de presión arterial sistólica varían en función de la edad, el rango normal en adultos sanos se considera, generalmente, entre 90 y 130 mmHg. En las etapas iniciales de la insuficiencia cardíaca, la presión arterial sistólica puede estar dentro del rango normal o ligeramente elevada. Sin embargo, a medida que la enfermedad progresa y el corazón se debilita, la presión arterial sistólica tiende a disminuir.  
...
* No obstante, en términos de varianza, los modelos logist_aic_rep y logist_rfe son los que presentan menor varianza de los resultados.   

* Se observa que el modelo con menor número de variables seleccionadas es logist_bic_rep. Este modelo es de los que mayor tasa de fallos presenta en sus predicciones.   
...
* El modelo con menor número de variables seleccionadas es logist_bic_rep. Este modelo tiene un AUC menor que otros.   


Con todo esto y en vista de las pocas observaciones de la clase minoritaria de la variable objetivo, vamos a considerar el **modelo logist_mmpc** como **el modelo de regresión logística más tentativo**, ya que, en conjunto, presentan las características más apropiadas.   
...
* Además, este mismo modelo logist_mmpc presenta una varianza de los resultados menor.   
...
Nos quedamos con mtry = 4 para 'Rf', para no concurrir en sobreajuste y establecer un número oportuno de variables en cada árbol generado.   
...
ggplot(as.data.frame(baggingbis$err.rate), aes(x = seq(1, 5000), y = OOB)) +
  ...
  labs(title = "Tasa de error en función del parámetro ntree (Bagging)", x = "ntree", y = "Tasa de error") +
  theme_minimal()
...
ggplot(as.data.frame(rfbis$err.rate), aes(x = seq(1, 5000), y = OOB)) +
  geom_point(color = "black", alpha = 0.8) +
  labs(title = "Tasa de error en función del parámetro ntree (Rf)", x = "ntree", y = "Tasa de error") +
  theme_minimal()
...
ggplot(gbm_3$results, aes(x = n.trees, y = Accuracy)) +
  ...
  geom_point(color = "black", alpha = 0.8) +
  labs(title = "Accuracy en función del parámetro n.trees (Gbm)", x = "n.trees", y = "Accuracy") +
  theme_minimal()
...
* En términos de varianza, los modelos logist_mmpc, rf y gbm son los que presentan menor varianza de los resultados.   
...
ggplot(xgbm_3$results, aes(x = nrounds, y = Accuracy)) +
  geom_line(color = "black", alpha = 0.8) +
  geom_point(color = "black", alpha = 0.8) +
  labs(title = "Accuracy en función del parámetro nrounds (XGboost)", x = "nrounds", y = "Accuracy") +
  theme_minimal()
...
SVM_lineal <- train(mortalidad ~ edad + presion_arterial_sistolica + creatinina + sodio + crepitantes_no +
  ...
  tuneGrid = expand.grid(C = c(0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 25, 30, 50)), verbose = FALSE)
...
No se observa máximo de Accuracy, por lo que nos quedamos con **C = 0.1 para 'SVM (lineal)'**.   
...
En vista del gráfico, nos quedamos con **C = 0.5 y scale = 5 para 'SVM (polinomial)'** para evitar sobreajuste.    
...
* En términos de varianza, son los modelos svm_lineal y svm_polinomial los que presentan menor varianza de los resultados.   
...
Por esta razón se prefiere optar por un modelo más sencillo con buenas características también. En este sentido, el modelo logist_mmpc se adecúa a nuestras exigencias.   


## 18: Elección del mejor modelo.

Una vez se han graficado todos los modelos y se han tenido en cuenta diferentes aspectos de estos tales como la tasa de fallos, AUC y varianza al estimar las predicciones, así como la complejidad del modelo, podemos concluir qué **el mejor modelo para nuestros datos es el modelo logist_mmpc**.   
...
Por otra parte, aunque existen otros modelos ensamblados que presentan un AUC ligeramente mayor y una tasa de fallos menor al modelo logist_mmpc, no compensa complicar la elección del modelo por una pequeña mejora. Además, el número de observaciones de nuestra clase minoritaria de la variable objetivo nos limita la elección del modelo a uno no demasiado complejo.    

Por todas estas razones, el modelo logist_mmpc es el que mejor se va a adaptar a nuestros datos.  


Con estas explicaciones y en vista a los gráficos boxplot comentados durante todo el trabajo, **no se procede a realizar contraste de hipótesis entre ningún modelo**, puesto que se tiene certeza de que el modelo logist_mmpc es el mejor modelo.   
