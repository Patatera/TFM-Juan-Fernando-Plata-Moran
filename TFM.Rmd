...
Concretamente, se quiere **predecir la mortalidad a un año de pacientes que han sufrido una insuficiencia cardíaca del dataset HF**.   

Para ello, se procede a **implementar varios algoritmos de clasificación: 'Regresión Logística', 'Redes Neuronales', 'Bagging', 'Rf' (Random Forest), 'Gbm', 'XGboost', 'SVM', 'Ensamblados' y 'Árbol de decisión'** al conjunto HF.   
...
Los datos forman parte de un **conjunto de características de pacientes que han sufrido un episodio de insuficiencia cardíaca, así como de medicamentos suministrados a estos pacientes para prevenir futuros riesgos**, con 786 observaciones.

📚 **Detalle de variables**: <https://journals.viamedica.pl/cardiology_journal/article/view/CJ.2012.0108/18174>


# Metodología

La forma de proceder será la siguiente:  
 - 1. En primer lugar, se realizará un breve análisis exploratorio y descripción de las variables.   
 - 2. A continuación, se argumentará la razón de no efectuar submuestreo y se determinará el balanceamiento de la variable objetivo.   
 - 3. Después, se llevará a cabo la modificación de los datos: depuración, codificación, dummyficación...   
 - 4. Más adelante, se efecturá un chequeo inicial.  
 - 5. Tras esto, se estudiarán varios métodos de selección de variables.   
 - 6. Posteriormente, se obtendrá un modelo de regresión logística para cada método de selección de variables y se compararán los resultados obtenidos mediante un gráfico boxplot. Además, se tomarán decisiones sobre los modelos más prometedores teniendo en cuenta sesgo, varianza y número de variables.  
 - 7. Seguidamente, se tunearán distintas redes neuronales.  
 - 8. Luego, se comparará la mejor red con el resto de modelos añadiéndola al gráfico boxplot anterior.   
 - 9. En siguiente lugar, se procederá al tuneado de los algoritmos ‘Bagging’ y ‘Rf’.  
 - 10. En este apartado, se añadirán los mejores modelos ‘Bagging y ‘Rf’ a la comparativa con los modelos anteriores.
 - 11. De forma análoga, se realizará el tuneado del algoritmo ‘Gbm’.
 - 12. Nuevamente, se incluirá el modelo ‘Gbm’ en el gráfico boxplot citado, comparándolos con el resto de modelos. 
 - 13. Para continuar, se codificará el tuneado del algoritmo ‘XGboost’.
 - 14. Se incorporará este nuevo modelo a nuestro gráfico boxplot, realizando las comparaciones pertinentes.
 - 15. En esta sección, se tunearán los algoritmos ‘SVM (lineal)’, ‘SVM (polinomial)’ y ‘SVM (RBF)’.
 - 16. Como ya es habitual, se anexarán los tres modelos SVM al gráfico boxplot mencionado, con su correspondiente comparativa con los modelos ya nombrados.
 - 17. Además, se construirán modelos ensamblados y se implementarán los más interesantes en nuestro conocido gráfico boxplot.
 - 18. Una vez llegados a este punto, se escogerá el mejor modelo para nuestros datos.
 - 19. Entonces, se expondrán las características de nuestro modelo elegido.
 - 20. Como última opción, se modelizará un árbol de decisión.
 - 21. Se representarán gráficos con el fin de obtener un enfoque visual de nuestro modelo.
 - 22. Finalmente, se darán unas breves conclusiones y reflexión personal.    
...
Se podría haber realizado más adelante, pero hemos preferido llevarla a cabo al inicio con el fin de realizar una exploración de las variables lo más adecuada posible.  

## 1: Análisis exploratorio y descripción de las variables.
...
* `cardiopatia_isquemica`: ¿El paciente ha sufrido una cardiopatía isquémica?  
* `fibrilacion_auricular`: ¿El paciente ha presentado fibrilación auricular?  
...
* `crepitantes`: ¿El paciente ha presentado crepitantes?  
* `tercer_tono_cardiaco`: ¿El paciente ha presentado tercer tono cardíaco?  
* `ingurgitacion_yugular`: ¿El paciente ha sufrido ingurgitación yugular?   
* `hepatomegalia`: ¿El paciente ha sufrido hepatomegalia?  
* `reflujo_hepatoyugular`: ¿El paciente ha sufrido reflujo hepatoyugular?   
* `edemas`: ¿El paciente ha presentado edemas?.  
...
* `inhibidores_ECA`: ¿Se le ha suministrado medicamentos inhibidores de la ECA al paciente?   
* `betabloqueadores`: ¿Se le ha suministrado medicamentos betabloqueadores al paciente?  
* `antagonistas_receptores_angiotensina_II`: ¿Se le ha suministrado medicamentos antagonistas de los receptores de angiotensina II al paciente?   
* `antagonistas_calcio`: ¿Se le ha suministrado medicamentos antagonistas de calcio al paciente?  
* `antiagregantes`: ¿Se le ha suministrado antiagregantes al paciente?  
* `anticoagulantes_orales`: ¿Se le ha suministrado anticoagulantes (orales) al paciente?  
* `digoxina`: ¿Se le ha suministrado digoxina al paciente?   
* `diureticos`: ¿Se le ha suministrado diuréticos al paciente?   
* `estatinas`: ¿Se le ha suministrado estatinas al paciente?  
* `mononitrato_isosorbida`: ¿Se le ha suministrado mononitrato de isosorbida al paciente?  
* `valvulopatias`: ¿El paciente ha presentado valvulopatías?  
* `mortalidad`: ¿El paciente falleció al año de sufrir el primer episodio de insuficiencia cardíaca?   

Además, se pueden **extraer algunas estadísticas básicas** de nuestros datos:  
...
Un primer vistazo a este resumen nos refleja que **no parece que haya problemas de rango**. **No hay valores ausentes, pero sí outliers** (a la vista de los histogramas). También se observa que **no todas las variables predictoras son numéricas, por lo que nos tocará dummyficar**.  

El tipo de cada variable ya se ha visualizado con la misma función `skim()`:  

 - Hay **21 variables de tipo carácter**.  
 - Hay **7 variables de tipo numérico**.  
 
Se observa que **todas las variables de tipo carácter son categóricas**. Habrá que **convertirlas a factor**.  

Analicemos las variables predictoras más en profundidad:  
...
ggplot(HF, aes(x = edad, color = mortalidad, fill = mortalidad)) +
  ...
  labs(title = "Diagrama variable Edad", x = "Edad", y = "Frecuencia") + 
  theme_minimal()
...
La mayoría de pacientes tienen entre 60 y 89 años.  
...
Hay pocas más mujeres que hombres, aunque el porcentaje de fallecidos en ambos sexos es muy parecido.  

```{r eval = TRUE}
# Análisis de la variable "cardiopatia_isquemica":
...
# Gráfico para la variable "cardiopatia_isquemica":
...
Sólo un 15% de los pacientes registrados han sufrido dicho síntoma.   

```{r eval = TRUE}
# Análisis de la variable "fibrilacion_auricular":
...
# Gráfico para la variable "fibrilacion_auricular":
...
Casi un 30% de los pacientes registrados han sufrido fibrilación auricular.  

```{r eval = TRUE}
# Análisis de la variable "presion_arterial_sistolica":
...
# Gráfico para la variable "presion_arterial_sistolica":

ggplot(HF, aes(x = presion_arterial_sistolica, color = mortalidad, fill = mortalidad)) +
  geom_histogram(alpha = 0.8) +
  labs(title = "Diagrama variable Presion_arterial_sistolica", x = "Presion_arterial_sistolica", 
  y = "Frecuencia") + 
  theme_minimal()
...
Aunque los valores de presión arterial sistólica varían en función de edad, el rango normal en adultos sanos se considera, generalmente, entre 90 y 130 mmHg. En las etapas iniciales de la insuficiencia cardíaca, la presión arterial sistólica puede estar dentro del rango normal o ligeramente elevada. Sin embargo, a medida que la enfermedad progresa y el corazón se debilita, la presión arterial sistólica tiende a disminuir.  

Podemos observar que gran parte de los pacientes registrados han sobrepasado este límite, lo cual es lógico al haber sufrido un episodio de insuficiencia cardíaca.  

```{r eval = TRUE}
# Análisis de la variable "presion_arterial_diastolica":
...
# Gráfico para la variable "presion_arterial_diastolica":
...
De forma análoga a la variable anterior, aunque los valores de presión arterial diastólica varían en función de edad, sí que hay un cierto límite a partir del cual hablamos de hipertensión arterial: 80 mmHg.  

Se observa que algo menos de la mitad de pacientes registrados han sobrepasado este límite.  

```{r eval = TRUE}
# Análisis de la variable "crepitantes":
...
# Gráfico para la variable "crepitantes":
...
Más de 2/3 de los pacientes han presentado tal síntoma.   

```{r eval = TRUE}
# Análisis de la variable "tercer_tono_cardiaco":
...
# Gráfico para la variable "tercer_tono_cardiaco":
...
Apenas un 5.4% de los pacientes registrados han presentado tercer tono cardíaco.    

```{r eval = TRUE}
# Análisis de la variable "ingurgitacion_yugular":
...
# Gráfico para la variable "ingurgitacion_yugular":
...
Sólo un 20.9%, aproximadamente, de los pacientes registrados han sufrido dicho síntoma.  

```{r eval = TRUE}
# Análisis de la variable "hepatomegalia":
...
# Gráfico para la variable "hepatomegalia":
...
Tan sólo un 11.5% de los pacientes registrados han sufrido este síntoma.  

```{r eval = TRUE}
# Análisis de la variable "reflujo_hepatoyugular":
...
# Gráfico para la variable "reflujo_hepatoyugular":
...
Se puede observar que los pacientes que han presentado tal síntoma no llegan al 6% de los totales registrados.  

```{r eval = TRUE}
# Análisis de la variable "edemas":
...
# Gráfico para la variable "edemas":
...
Más de 2/3 de los pacientes registrados presentaron edemas.  

```{r eval = TRUE}
# Análisis de la variable "creatinina":
...
# Gráfico para la variable "creatinina":
...
En nuestro caso, casi el 25.5% de los pacientes hombres registrados presentaron niveles de creatinina en sangre superiores a 1.3 mg/dL. Por otra parte, poco más del 38.6% de las pacientes mujeres registradas han presentado niveles de creatinina en sangre superiores a 1.1 mg/dL.   

```{r eval = TRUE}
# Análisis de la variable "sodio":
...
# Gráfico para la variable "sodio":
...
Los niveles normales de sodio en sangre se estiman entre 135 y 145 mEq/L. Aquellos niveles inferiores a 135 mEq/L o superiores a 145 mEq/L pueden ser síntoma de haber padecido una insuficiencia cardíaca.  

Poco más del 17.8% de los pacientes registrados han presentado niveles de sodio en sangre anormales.  

```{r eval = TRUE}
# Análisis de la variable "potasio":
...
# Gráfico para la variable "potasio":
...
Nuevamente, los niveles normales de potasio en sangre se estiman entre 3.7 y 5.2 mEq/L. Cualquier valor por debajo de 3.7 mEq/L o por encima de 5.2 mEq/L puede provocar alteraciones en la función cardíaca que pueden llevar a la insuficiencia cardíaca.  

En nuestro registro, algo más del 19.4% de los pacientes presentaron niveles anormales.  

```{r eval = TRUE}
# Análisis de la variable "hemoglobina":
...
# Gráfico para la variable "hemoglobina":
...
En nuestros datos, algo más de la mitad de los pacientes hombres registrados presentaron unos niveles de hemoglobina en sangre menores a 13.2 g/dL y, aproximadamente el 31.4% de las pacientes mujeres registradas presentaron unos niveles de hemoglobina en sangre menores a 11.6 g/dL.

```{r eval = TRUE}
# Análisis de la variable "inhibidores_ECA":
...
A más de la mitad de los pacientes registrados se les suministraron medicamentos inhibidores de la ECA.  

```{r eval = TRUE}
# Análisis de la variable "betabloqueadores":
...
# Gráfico para la variable "betabloqueadores":
...
```{r eval = TRUE}
# Análisis de la variable "antagonistas_receptores_angiotensina_II":
...
# Gráfico para la variable "antagonistas_receptores_angiotensina_II":
...
```{r eval = TRUE}
# Análisis de la variable "antagonistas_calcio":
...
# Gráfico para la variable "antagonistas_calcio":
...
```{r eval = TRUE}
# Análisis de la variable "antiagregantes":
...
# Gráfico para la variable "antiagregantes":
...
```{r eval = TRUE}
# Análisis de la variable "anticoagulantes_orales":
...
# Gráfico para la variable "anticoagulantes_orales":
...
Las estadísticas nos muestran que, prácticamente, al 35.5% de los pacientes registrados se les suministraron anticoagulantes orales.   

```{r eval = TRUE}
# Análisis de la variable "digoxina":
...
# Gráfico para la variable "digoxina":
...
Casi al 30% de los pacientes registrados se les suministró digoxina.   

```{r eval = TRUE}
# Análisis de la variable "diureticos":
...
# Gráfico para la variable "diureticos":
...
En este caso, al 74% de los pacientes registrados se les suministraron diuréticos.

```{r eval = TRUE}
# Análisis de la variable "estatinas":
...
# Gráfico para la variable "estatinas":
...
Las estatinas fueron suministradas a algo más del 30.5% de los pacientes registrados.  

```{r eval = TRUE}
# Análisis de la variable "mononitrato_isosorbida":
...
# Gráfico para la variable "mononitrato_isosorbida":
...
Según los registros, no más del 10% de los pacientes fueron suministrados con mononitrato de isosorbida.   

```{r eval = TRUE}
# Análisis de la variable "valvulopatias":
...
# Gráfico para la variable "valvulopatias":
...
Finalmente, presentaron valvulopatías casi un 30% de los pacientes registrados.  


A continuación, se procede a ejecutar un contraste de independencia para tener mayor certeza de si las variables categóricas son dependientes o no de la variable objetivo:  
...
**Aunque hay variables con p-valores mayores a 0.05**, que darían evidencias suficientes para decir que son independientes de la variable objetivo, **proponemos no eliminar ninguna variable** con el fin de realizar un modelo lo más fiel posible a todos los datos ofrecidos.  

Posteriormente, se procederá a comprobar posibles problemas de colinealidad entre posibles variables numéricas:   

```{r eval = TRUE}
# Se ve la colinealidad entre las variables numéricas:
...
## Se obtiene un gráfico de correlación:
...
Según se observa, **no parece existir una correlación alta entre ninguna variable numérica**.  
...
Vemos que **la variable objetivo está desbalanceada**. Las pocas observaciones de cada clase pueden complicar el tuneado de algunos algoritmos, como la red neuronal.  


## 3: Modificación de los datos.

Una vez llevado a cabo el análisis de todas las variables predictoras, se deberán tomar **dos tipos decisiones**:   

* **las que afectan a la base de datos en general**: problemas de codificación o rango, creación de variables en general, variables que no aportan, pasar a factores, etc.   

* **las que afectan a un algoritmo en concreto**: recategorización, tratamiento de ausentes, estandarización, dummyficación, etc.    
...
HF <- HF |> 
  # Conversión de variables de tipo carácter a factor:
...
HF_rec <-
  ...
  # Recategorización de niveles con menos de 20 observaciones en una categoría:
  step_other(all_nominal_predictors(), threshold = 20, other = "other") |> 
  # Aplicación del filtro de correlación a variables numéricas:
  step_corr(all_numeric_predictors(), threshold = 0.9) |> 
  # Normalización por rango a las variables numéricas:
  step_range(all_numeric_predictors()) |> 
  # Dummyficación de las variables categóricas:
  step_dummy(all_nominal_predictors()) |> 
  # Eliminación de las categorías  con menos de 20 observaciones:
  step_rm(contains("other"))


# Comprobación de la receta:
...
Antes de llevar a cabo la selección de variables, vamos a realizar un chequeo intuitivo ejecutando:   

* Un modelo logístico inicial con variables obtenidas con stepwise básico.   
* Un modelo rf inicial.  

En primer lugar, se procede a obtener las variables del modelo logístico inicial con stepwise básico:   

```{r eval = TRUE}
# Stepwise básico para obtener las variables del modelo logístico inicial:
...
Una vez tenemos las variables seleccionadas, se entrena el modelo logístico inicial y se saca la matriz de confusión y la curva ROC:   

```{r eval = TRUE}
# Entrenamos el modelo logístico inicial con las variables seleccionadas del stepwise básico:
...
En segundo lugar, se entrena el modelo rf inicial, sacando la matriz de confusión y la curva ROC:  
...
Como ya se ha comentado, hay 139 observaciones de la clase minoritaria (Yes) de la variable objetivo (17.7% de las observaciones totales) por lo que no se permitirán ni modelos muy complicados ni con muchas variables predictoras.  

En vista a los resultados obtenidos, no merece la pena un algoritmo más complejo, sino que se prefiere la logística.  

Por otro lado, no se aprecia una alta tasa de pacientes fallecidos clasificados correctamente en ninguna de las dos matrices de confusión (la sensibilidad obtenida es bastante baja). Esto puede deberse al alto desbalanceamiento de la variable objetivo o a la falta de buenas variables predictoras, entre otros factores.    

Por todo esto se propondrá cambiar el punto de corte para intentar detectar más pacientes fallecidos correctamente e intentar mejorar nuestro modelo final elegido.   


## 5: Selección de variables.

A continuación, se proponen varios métodos de selección de variable:  
...
Este método SBF selecciona 11 variables:  
...
Este método RFE seleccionaba más variables, pero tras la visualización de los resultados en gráfico seleccionamos las siguientes: 
...
Este método AIC selecciona 12 variables:
...
Este método BIC selecciona 3 variables:
...
Este método Boruta selecciona 4 variables:
...
Este método MMPC selecciona 6 variables:
...
Este método SES selecciona también las mismas 6 variables:
...
Este método AIC repetido selecciona 10 variables:
...
Este método BIC repetido selecciona 2 variables:   

"edad", "creatinina".   

Este método BIC repetido 2 selecciona 3 variables:

"edad", "creatinina", "presion_arterial_sistolica".   


Si presentamos **todos los resultados en una tabla**:

```{r eval = TRUE}
# Tabla con todos los métodos de selección de variables:

tabla <- as_tibble(cbind(Metodo = c("SBF", "RFE", "AIC", "BIC", "Boruta", "MMPC", "SES", "AICrep", "BICrep",        "BICrep 2"), Num_variables = c(length(variables_sbf), 8, length(variables_aic) - 1, length(variables_bic) - 1,
  length(variables_boruta), length(variables_mmpc), length(variables_ses), length(variables_aic_rep),
  length(variables_bic_rep), length(variables_bic_rep2)), Variables = c("")))
...
Con cada método de selección de variables se obtiene un modelo. Se comparan estos modelos para ver cuál es el más adecuado mediante validación cruzada repetida. Los resultados se reflejan en el boxplot siguiente:   

```{r eval = TRUE}
# Comparación de modelos vía validación cruzada repetida:  

source("cruzadas avnnet y log binaria.R")
detach("package:MXM")
detach("package:Boruta")

## Modelo Logística (SBF):
...
modelo_logistica_sbf$modelo = "Logística (SBF)"

## Modelo Logística (RFE):
...
modelo_logistica_rfe$modelo = "Logística (RFE)"

## Modelo Logística (AIC):
...
modelo_logistica_aic$modelo = "Logística (AIC)"

## Modelo Logística (BIC):
...
modelo_logistica_bic$modelo = "Logística (BIC)"
...
modelo_logistica_boruta$modelo = "Logística (Boruta)"

## Modelo Logística (MMPC):
...
modelo_logistica_mmpc$modelo = "Logística (MMPC)"

## Modelo Logística (AICrep):
...
modelo_logistica_aic_rep$modelo = "Logística (AICrep)"

## Modelo Logística (BICrep):
...
modelo_logistica_bic_rep$modelo = "Logística (BICrep)"
...
# Tabla con los modelos seleccionados:

tabla_2 <- as_tibble(cbind(Modelo = c("Logística (AIC)", "Logística (AICrep)", "Logística (BIC)", "Logística        (BICrep)", "Logística (Boruta)", "Logística (MMPC)", "Logística (RFE)", "Logística (SBF)"), 
  Num_variables = c(length(variables_aic) - 1, length(variables_aic_rep), length(variables_bic) - 1,
  length(variables_bic_rep), length(variables_boruta), length(variables_mmpc), 8, length(variables_sbf)),
  Variables = c("")))
...
Con respecto al **primer gráfico boxplot**: 

* El modelo Logística (MMPC) es el que menor tasa de fallos presenta en sus predicciones.

* No obstante, en términos de varianza, los modelos Logística (AICrep) y Logística (RFE) son los que presentan menor varianza de los resultados.   

* Se observa que el modelo con menor número de variables seleccionadas es Logística (BICrep). Este modelo es de los que mayor tasa de fallos presenta en sus predicciones.   


En el **segundo gráfico boxplot**:

* El modelo Logística (AICrep) es el que mayor AUC presenta.  

* Sin embargo, el modelo Logística (BICrep) es el que presenta menor varianza de los resultados.  

* El modelo con menor número de variables seleccionadas es Logística (BICrep). Este modelo tiene un AUC menor que otros.   


Con todo esto y en vista de las pocas observaciones de la clase minoritaria de la variable objetivo, vamos a considerar el **modelo Logística (MMPC)** como **el modelo de regresión logística más tentativo**, ya que, en conjunto, presentan las características más apropiadas.   


## 7: Tuneado de redes neuronales (con método MMPC).

Con las características de nuestro modelo Logística (MMPC), se procede a tunear varias redes neuronales:   

* 6 variables.   
* 139 obs en la clase de interés (Yes).  
* 22 obs / parámetro (ya que tenemos 6 variables).   

Entonces, se determinan los parámetros máximos a estimar según la expresión: 139 / 22 ≈ 6 parámetros max.   

Ahora, mediante la fórmula siguiente se calculan cuántos nodos máximos tendrá cada red neuronal:   

6 = h * (k + 1) + h + 1   

* h es el número máximo de nodos.  
* k es el número de variables.   

Esto implica: h = 5 / (k + 2) = 5 / 8 ≈ 1 nodos max.   

Sin embargo, se pondrán 5 nodos, que es un mínimo aceptable para una red. Con lo que queda:  

* 5 nodos max.  
* 41 parámetros max.  
* 3 obs / parámetro.   

Por esta razón no cabe tunear el número de nodos. No hay margen y un número más alto sobreajustará.  

Llegados a este punto, se procede al tuneado de redes:   

```{r eval = TRUE}
# Tuneado de parámetros maxit y decay en redes con las variables seleccionadas del método MMPC:
...
for (iter in listaiter) {
  ...
  # Adicción de la columna del parametro de iteraciones:
...
A la vista del gráfico de puntos, llega un momento en que la red se sobreajusta, por lo que es necesario early stopping.   

En nuestro caso, se propone dejar el tuneado en **maxit = 100, decay = 0.01**.   


## 8: Comparación de modelos con la mejor red.

En este apartado, se añade la mejor red neuronal a la comparativa con los mejores modelos de regresión logística:    
...
modelo_rednnet$modelo = "Red neuronal"
...
tabla_3 <- as_tibble(cbind(Modelo = c("Logística (MMPC)", "Red neuronal"), 
  Num_variables = c(length(variables_mmpc), length(variables_mmpc)), Variables = c("")))
...
Observando el **primer gráfico boxplot**:  

* El modelo Logística (MMPC) es el que menor tasa de fallos presenta en sus predicciones.  

* En términos de varianza, es el mismo modelo el que presenta una menor varianza de los resultados.   


Si nos fijamos en el **segundo gráfico boxplot**:  

* El modelo Logística (MMPC) es el que mayor AUC presenta.   

* Además, este mismo modelo Logística (MMPC) presenta una varianza de los resultados menor.   


## 9: Tuneado de Bagging y Rf.

Estos son otros algoritmos de clasificación.   

Procedemos al tuneado de parámetros de estos algoritmos, tales como mtry (sólo para 'Rf') y sampsize, así como a la toma de decisión del mínimo ntree.   

Empezamos por tuneado de mtry (sólo para 'Rf'):   

```{r eval = TRUE}
# Tuneado del parámetro mtry en Rf con todas las variables:
...
Este orden de importancia de variables puede servir de base para la selección de variables de los algoritmos 'Bagging' y 'Rf'. Por esta razón, y en base a las variables que más bajan el Accuracy si no se usan, seleccionaremos las siguientes variables:   
...
```{r eval = TRUE}
# Tuneado del parámetro mtry en Rf con las variables seleccionadas:
...
rf_2 <- train(mortalidad ~ edad + creatinina + presion_arterial_sistolica + sodio + hemoglobina + inhibidores_ECA_si,
  ...
  savePredictions = "all", classProbs = TRUE), tuneGrid = expand.grid(mtry = c(4, 5, 7, 12, 14, 15, 31)),
...
Nos quedamos con mtry = 4 para 'Rf', puesto que es el proporciona un Accuracy mayor.   


En siguiente lugar, vamos a decidir el mínimo número de ntree para 'Bagging':
...
plot(baggingbis$err.rate[, 1])


```


Se observa que, aproximadamente, con ntree = 300 se obtiene un error menor en las predicciones. Por lo que se decide dejar este mínimo número de ntree para 'Bagging'.   


A continuación, se tunea el parámetro sampsize en 'Bagging' según:

* 4 grupos.  
* 786 observaciones.   
* (3/4) * 786 = 589  -- max valor del parámetro sampsize (que prueba caret por defecto).    
...
modelo_bagging_2$modelo = "Bagging 2"
...
modelo_bagging_3$modelo = "Bagging 3"
...
modelo_bagging_4$modelo = "Bagging 4"
...
modelo_bagging_5$modelo = "Bagging 5"
...
modelo_bagging_6$modelo = "Bagging 6"
...
modelo_bagging_7$modelo = "Bagging 7"
...
modelo_bagging_base$modelo = "Bagging BASE"
...
Por otro lado, decidamos el mínimo número de ntree para 'Rf':  
...
En este caso, con ntree = 200 se obtiene un error menor en las predicciones. Por lo que se decide dejar este mínimo número de ntree para 'Rf'.  


A continuación, se tunea el parámetro sampsize en 'Rf' según:
...
Nos quedamos con **mtry = 4, ntree = 200 y sampsize = 50 para 'Rf'**, puesto que, en conjunto, es el modelo que mejores características presenta en relación a la tasa de fallos, AUC y varianza de los resultados. Es decir, se elige el **modelo Rf**.  


## 10: Comparación de modelos con Bagging y Random forest.

En este apartado, se añaden el **modelo Bagging** y el **modelo Rf** a la comparativa con el resto de modelos:   
...
tabla_4 <- as_tibble(cbind(Modelo = c("Bagging", "Logística (MMPC)", "Red neuronal", "Rf"), 
  Num_variables = c(6, length(variables_mmpc), length(variables_mmpc), 6), Variables = c("")))
...
* A simple vista, son los modelos Logística (MMPC) y Rf los que presentan menor varianza de los resultados.   
 

Si nos fijamos en el **segundo gráfico boxplot**:  

* Los modelos Logística (MMPC) y Rf son los que mayor AUC presentan.   
...
Este es otro algoritmo de clasificación.   

Procedemos al tuneado de parámetros de este algoritmo, tales como bag.fraction, shrinkage, n.minobsinnode y n.trees.   

En relación al parámetro bag.fraction lo quedaremos en bag.fraction = 1 ya que tenemos pocas observaciones y nos conviene que nuestro modelo aprenda de las máximas observaciones posibles que sirven de entrenamiento.

Continuamos por tuneado de shrinkage, n.minobsinnode y n.trees:  

```{r eval = TRUE}
# Tuneado de parámetros shrinkage, n.minobsinnode y n.trees en Gbm con todas las variables:
...
Este orden de importancia de variables puede servir de base para la selección de variables del algoritmo 'Gbm'. Por esta razón, y en base a las variables que más información relacionada con la variable objetivo presentan, seleccionaremos las siguientes variables:  
...
```{r eval = TRUE}
# Tuneado de parámetros shrinkage, n.minobsinnode y n.trees en Gbm con las variables seleccionadas:
...
Aunque el entrenamiento nos ofrece como parámetros óptimos shrinkage = 0.05 y n.minobsinnode = 10, si miramos las gráficas obtenidas, parece más fiable elegir los parámetros shrinkage = 0.05 y n.minobsinnode = 20.  


Vamos a hacer estudio de Early stopping para evitar un posible sobreajuste del parámetro n.trees:  
...
## Fijamos los parámetros shrinkage y n.minobsinnode para ver como evoluciona en función del parámetro n.trees:
...
Vemos que el Accuracy tiene varios máximos locales, por lo que finalmente nos quedamos con **shrinkage = 0.05, n.minobsinnode = 20 y n.trees = 75 para 'Gbm'**


## 12: Comparación de modelos con Gbm.

En este apartado, se añade el modelo 'Gbm' a la comparativa con el resto de modelos:   
...
tabla_5 <- as_tibble(cbind(Modelo = c("Bagging", "Gbm", "Logística (MMPC)", "Red neuronal", "Rf"), 
  Num_variables = c(6, 4, length(variables_mmpc), length(variables_mmpc), 6), Variables = c("")))
...
* En términos de varianza, los modelos Logística (MMPC), Rf y Gbm son los que presentan menor varianza de los resultados.   
...
* El modelo con menor número de variables seleccionadas es Gbm. Este modelo tiene un AUC menor que otros.   
...
Procedemos al tuneado de parámetros de este algoritmo, tales como min_child_weight, eta y nrounds:  

```{r eval = TRUE}
# Tuneado de parámetros min_child_weight, eta y nrounds en XGboost con todas las variables: 
...
Este orden de importancia de variables puede servir de base para la selección de variables del algoritmo 'XGboost'. Por esta razón, seleccionaremos las siguientes variables: 
...
```{r eval = TRUE}
# Tuneado de parámetros min_child_weight, eta y nrounds en XGboost con las variables seleccionadas: 
...
Aunque el entrenamiento nos ofrece como parámetros óptimos min_child_weight = 20 y eta = 0.001, si miramos las gráficas obtenidas, parece más fiable elegir los parámetros min_child_weight = 20 y eta = 0.05.  


Vamos a hacer estudio de Early stopping para evitar un posible sobreajuste del parámetro nrounds:  
...
## Fijamos los parámetros min_child_weight y eta para ver como evoluciona en función del parámetro nrounds:
...
## 14: Comparación de modelos con XGboost.

En este apartado, se añade el modelo 'XGboost' a la comparativa con el resto de modelos:   
...
tabla_6 <- as_tibble(cbind(Modelo = c("Bagging", "Gbm", "Logística (MMPC)", "Red neuronal", "Rf", "XGboost"),
  Num_variables = c(6, 4, length(variables_mmpc), length(variables_mmpc), 6, 8), 
  Variables = c("")))
...
* En términos de varianza, es el modelo XGboost el que presenta menor varianza de los resultados.   


Si nos fijamos en el **segundo gráfico boxplot**:  

* Los modelos Logística (MMPC) y Rf son los que mayor AUC presenta.   
...
Procedemos al tuneado del parámetro C de este algoritmo:   

```{r eval = TRUE}
# Tuneado del parámetro C en SVM (lineal) con las variables seleccionadas del método MMPC:
...
No se observa máximo de Accuracy, por lo que nos quedamos con **C = 50 para 'SVM (lineal)'**.   
...
Procedemos al tuneado del parámetro C y scale de este algoritmo:   

```{r eval = TRUE}
# Tuneado de parámetros C y scale en SVM (polinomial) con las variables seleccionadas del método MMPC: 
...
SVM_poly <- train(mortalidad ~ edad + presion_arterial_sistolica + creatinina + sodio + crepitantes_no +
  ...
  repeats = 5, savePredictions = "all", classProbs = TRUE),
  tuneGrid = expand.grid(C = c(0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 30, 50), degree = 2,
  scale = c(0.1, 0.5, 1, 2, 5)), verbose = FALSE)

SVM_poly
SVM_poly$results
ggplot(SVM_poly$results, aes(x = factor(C), y = Accuracy, color = factor(degree), pch = factor(scale))) +
  geom_point(position = position_dodge(width = 0.5), size = 3)
...
En vista del gráfico, nos quedamos con **C = 50 y scale = 5 para 'SVM (poly)'** para evitar sobreajuste.    
...
Procedemos al tuneado del parámetro C y sigma de este algoritmo:  

```{r eval = TRUE}
# Tuneado de parámetros C y sigma en SVM (RBF) con las variables seleccionadas del método MMPC:
...
SVM_radial <- train(mortalidad ~ edad + presion_arterial_sistolica + creatinina + sodio + crepitantes_no +
  ...
  repeats = 5, savePredictions = "all", classProbs = TRUE),
  tuneGrid = expand.grid(C = c(0.01, 0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 30, 50), sigma = c(0.0001, 0.005, 0.01, 0.05)),
  verbose = FALSE)
...
Nos quedamos con **C = 10 y sigma = 0.01 para 'SVM (RBF)'**.   


## 16: Comparación de modelos con SVM (lineal, polinomial y radial).

En este apartado, se añaden los distintos modelos de 'SVM' a la comparativa con el resto de modelos: 
...
modelo_svm_lineal <- cruzadaSVMbin(data = HF_prep, vardep = "mortalidad", listconti = c("edad",                             "presion_arterial_sistolica", "creatinina", "sodio", "crepitantes_no", "inhibidores_ECA_si"), listclass = c(""),
  grupos = 4, sinicio = 1234, repe = 10, C = 50)

modelo_svm_lineal$modelo = "SVM (lineal)"
...
modelo_svm_polinomial <- cruzadaSVMbinPoly(data = HF_prep, vardep = "mortalidad", listconti = c("edad",                     "presion_arterial_sistolica", "creatinina", "sodio", "crepitantes_no", "inhibidores_ECA_si"), listclass = c(""),
  grupos = 4, sinicio = 1234, repe = 10, C = 50, degree = 2, scale = 5)

modelo_svm_polinomial$modelo = "SVM (polinomial)"
...
modelo_svm_rbf <- cruzadaSVMbinRBF(data = HF_prep, vardep = "mortalidad", listconti = c("edad",                             "presion_arterial_sistolica", "creatinina", "sodio", "crepitantes_no", "inhibidores_ECA_si"), listclass = c(""),
  grupos = 4, sinicio = 1234, repe = 10, C = 10, sigma = 0.01)

modelo_svm_rbf$modelo = "SVM (RBF)"
...
tabla_7 <- as_tibble(cbind(Modelo = c("Bagging", "Gbm", "Logística (MMPC)", "Red neuronal", "Rf", "SVM (lineal)",
  "SVM (polinomial)", "SVM (RBF)", "XGboost"), Num_variables = c(6, 4, length(variables_mmpc),
  length(variables_mmpc), 6, length(variables_mmpc), length(variables_mmpc), length(variables_mmpc), 8),
  Variables = c("")))
...
* En términos de varianza, son los modelos SVM (lineal) y SVM (polinomial) los que presentan menor varianza de los resultados.   
...
* El modelo con menor número de variables seleccionadas es Gbm. Este modelo es de los que mayor AUC presentan.   
...
modelo_logistica_mmpc_ensamblado_resultados$modelo <- "Logística (MMPC)"
modelo_logistica_mmpc_ensamblado_predicciones <- as.data.frame(modelo_logistica_mmpc_ensamblado[2])
modelo_logistica_mmpc_ensamblado_predicciones$logistica_mmpc <- modelo_logistica_mmpc_ensamblado_predicciones$Yes
...
modelo_rednnet_ensamblado_resultados$modelo <- "Red neuronal"
...
modelo_svm_lineal_ensamblado_resultados$modelo <- "SVM (lineal)"
...
modelo_svm_polinomial_ensamblado_resultados$modelo <- "SVM (polinomial)"
...
modelo_svm_rbf_ensamblado_resultados$modelo <- "SVM (RBF)"
...
unipredi$predi10 <- (unipredi$logistica_mmpc + unipredi$rednnet) / 2
unipredi$predi11 <- (unipredi$logistica_mmpc + unipredi$bagging) / 2
unipredi$predi12 <- (unipredi$logistica_mmpc + unipredi$rf) / 2
unipredi$predi13 <- (unipredi$logistica_mmpc + unipredi$gbm) / 2
unipredi$predi14 <- (unipredi$logistica_mmpc + unipredi$xgboost) / 2
unipredi$predi15 <- (unipredi$logistica_mmpc + unipredi$svm_lineal) / 2
unipredi$predi16 <- (unipredi$logistica_mmpc + unipredi$svm_polinomial) / 2
unipredi$predi17 <- (unipredi$logistica_mmpc + unipredi$svm_rbf) / 2
...
unipredi$predi43 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$bagging) / 3
unipredi$predi44 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$rf) / 3
unipredi$predi45 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$gbm) / 3
unipredi$predi46 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$xgboost) / 3
unipredi$predi47 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$svm_lineal) / 3
unipredi$predi48 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$svm_polinomial) / 3
unipredi$predi49 <- (unipredi$logistica_mmpc + unipredi$rednnet + unipredi$svm_rbf) / 3
unipredi$predi50 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$rf) / 3
unipredi$predi51 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$gbm) / 3
unipredi$predi52 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$xgboost) / 3
unipredi$predi53 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$svm_lineal) / 3
unipredi$predi54 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$svm_polinomial) / 3
unipredi$predi55 <- (unipredi$logistica_mmpc + unipredi$bagging + unipredi$svm_rbf) / 3
unipredi$predi56 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$gbm) / 3
unipredi$predi57 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$xgboost) / 3
unipredi$predi58 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$svm_lineal) / 3
unipredi$predi59 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$svm_polinomial) / 3
unipredi$predi60 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$svm_rbf) / 3
unipredi$predi61 <- (unipredi$logistica_mmpc + unipredi$gbm + unipredi$xgboost) / 3
unipredi$predi62 <- (unipredi$logistica_mmpc + unipredi$gbm + unipredi$svm_lineal) / 3
unipredi$predi63 <- (unipredi$logistica_mmpc + unipredi$gbm + unipredi$svm_polinomial) / 3
unipredi$predi64 <- (unipredi$logistica_mmpc + unipredi$gbm + unipredi$svm_rbf) / 3
unipredi$predi65 <- (unipredi$logistica_mmpc + unipredi$xgboost + unipredi$svm_lineal) / 3
unipredi$predi66 <- (unipredi$logistica_mmpc + unipredi$xgboost + unipredi$svm_polinomial) / 3
unipredi$predi67 <- (unipredi$logistica_mmpc + unipredi$xgboost + unipredi$svm_rbf) / 3
...
unipredi$predi86 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$bagging + unipredi$xgboost) / 4
unipredi$predi87 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$bagging + unipredi$gbm) / 4
unipredi$predi88 <- (unipredi$rf + unipredi$bagging + unipredi$xgboost + unipredi$gbm) / 4

unipredi$predi89 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$bagging + unipredi$xgboost + unipredi$gbm) / 5
unipredi$predi90 <- (unipredi$logistica_mmpc + unipredi$rf + unipredi$bagging + unipredi$xgboost + unipredi$rednnet) / 5
...
## Se obtiene el número de repeticiones CV y se calculan las medias por repe en el data frame medias0:
...
Una vez se han graficado todos los modelos, nos quedamos con aquellos de interés:   

```{r eval = TRUE}
# 9: GRÁFICOS BOXPLOT ELEGIDOS:

listadobis <- c("logistica_mmpc", "rednnet", "bagging", "rf", "gbm", "xgboost", "svm_lineal", "svm_polinomial",             "svm_rbf", "predi12", "predi13", "predi14", "predi22", "predi26", "predi31", "predi36", "predi51", "predi56",             "predi57", "predi58", "predi61", "predi70", "predi71", "predi74", "predi79", "predi87", "predi88", "predi90",             "predi91")
...
De nuevo, observando los dos gráficos boxplot se llega a la conclusión de que el modelo que se mejor ajusta a nuestros datos es el modelo predi51 puesto que es el que mejor balance establece entre una baja tasa de fallos, un alto AUC y una menor varianza de los resultados.   

Es un modelo ensamblado, fruto del promedio de los modelos Logística (MMPC), Bagging y Gbm.  

Sin embargo, debido al bajo número de observaciones que tiene nuestra clase minoritaria de la variable objetivo, no podemos permitirnos modelos tan complejos como el predi51.   

Por esta razón se prefiere optar por un modelo más sencillo con buenas características también. En este sentido, el modelo Logística (MMPC) se adecúa a nuestras exigencias.   


## 18: Elección del mejor modelo.

Una vez se han graficado todos los modelos y se han tenido en cuenta diferentes aspectos de estos tales como la tasa de fallos, AUC y varianza al estimar las predicciones, así como la complejidad del modelo, podemos concluir qué **el mejor modelo para nuestros datos es el modelo Logística (MMPC)**.   
...
En cuanto a la tasa de fallos, aunque el modelo Logística (MMPC) no es el que menor tasa de fallos presenta en sus predicciones, la diferencia con respecto a otros modelos con menor tasa de fallos en sus predicciones no es grande, por lo que se prefiere un modelo con mayor AUC.   

Por otra parte, aunque existen otros modelos ensamblados que presentan un AUC ligeramente mayor y una tasa de fallos menor al modelo Logística (MMPC), no compensa complicar la elección del modelo por una pequeña mejora. Además, el número de observaciones de nuestra clase minoritaria de la variable objetivo nos limita la elección del modelo a uno no demasiado complejo.    

Por todas estas razones, el modelo Logística (MMPC) es el que mejor se va a adaptar a nuestros datos.  


Con estas explicaciones y en vista a los gráficos boxplot comentados durante todo el trabajo, **no se procede a realizar contraste de hipótesis entre ningún modelo**, puesto que se tiene certeza de que el modelo Logística (MMPC) es el mejor modelo.   


## 19: Características de nuestro modelo elegido.

Se hace la matriz de confusión de nuestro modelo elegido Logística (MMPC) y se obtiene la curva ROC. En este caso, se opta por hacer validación cruzada con 10 grupos. Así se obtiene el error aproximado con el modelo construido con el 90% de las observaciones:   

```{r eval = TRUE}
# Entrenamos el modelo elegido Logística (MMPC) con las variables seleccionadas del método MMPC:
...
auc_modelo_elegido_logistica_mmpc <- curvaroc_modelo_elegido_logistica_mmpc$auc
auc_modelo_elegido_logistica_mmpc
plot(roc(response = sal_modelo_elegido_logistica_mmpc$obs, predictor = sal_modelo_elegido_logistica_mmpc$Yes))

  
```


Se observa en la matriz de confusión obtenida el problema para detectar correctamente pacientes que fallecen al año de sufrir una insuficiencia cardíaca.  

A continuación, se procede a comentar los resultados obtenidos:  

* La sensibilidad o tasa de verdaderos positivos se refiere a la proporción de pacientes fallecidos que son correctamente identificados o detectados por nuestro modelo Logística (MMPC). Concretamente, la sensibilidad de nuestro modelo Logística (MMPC) es bastante baja: 0.0576.   

* La especificidad o tasa de verdaderos negativos se refiere a la proporción de pacientes no fallecidos que son correctamente identificados o clasificados como no fallecidos. En nuestro caso, la especificidad del modelo Logística (MMPC) es cercana a 1: 0.9876.   

* El Accuracy o tasa de bien clasificados se refiere a la proporción de pacientes clasificados correctamente (tanto pacientes fallecidos como pacientes no fallecidos) sobre el total de pacientes. Se observa un Accuracy alto de nuestro modelo elegido: 0.8232.   

* La tasa de fallos se refiere a la proporción de pacientes clasificados incorrectamente (tanto pacientes fallecidos como pacientes no fallecidos) entre el total de pacientes. En este caso, se calcula una tasa de fallos de 0.1768.   

* La precisión es una medida que se refiere a la proporción de pacientes fallecidos identificados correctamente sobre el total de pacientes clasificados como fallecidos. Así, nuestra precisión es de 0.5.   

* El AUC se refiere al área bajo la curva ROC. La curva ROC es una representación gráfica que muestra la relación entre la sensibilidad y la especificidad de un modelo de clasificación binaria. En este caso, el AUC es de 0.6896.   


Con la intención de mejorar la sensibilidad de nuestro modelo, se propone rebajar el punto de corte. Con ello se asume que habrá más falsos positivos.   

El punto de corte establece el límite a partir del cual se clasifica a un paciente como perteneciente a la clase de pacientes fallecidos. 

Se hará uso del método Youden para encontrar el punto de corte óptimo que maximice la capacidad de discriminación del modelo, balanceando adecuadamente la sensibilidad y la especificidad:   
...
# Probamos el punto de corte óptimo:
...
Paralelamente, se ve que la precisión de nuestro modelo ha disminuido ligeramente a un valor de 0.2973 ya que, al disminuir el punto de corte, ha disminuido la proporción de pacientes fallecidos clasificados correctamente sobre el total de pacientes identificados como fallecidos.    

También nuestro Accuracy ha disminuido ya que hay menos pacientes totales clasificados correctamente.  


Si se comparan ambos resultados en una tabla:  

```{r eval = TRUE}
# Tabla con características del modelo final escogido:

tabla_8 <- as_tibble(cbind(Num_observaciones = rep(786, 2), Num_observaciones_clase_minoritaria = rep("139 (17.7%)", 2),
  Modelo_elegido = rep("Logística (MMPC)", 2),  Variable_objetivo = rep("mortalidad", 2), 
  Variables_predictoras = rep("edad, presion_arterial_sistolica, creatinina, sodio, crepitantes_no, inhibidores_ECA_si",
  2), Num_variables_predictoras = rep(length(variables_mmpc), 2), Punto_corte = c(0.5, 0.2), Tasa_fallos = c(""),
  AUC = rep(0.6896, 2), Sensibilidad = c(""), Especificidad = c(""), Accuracy = c(""),
  Precision = c(""), Num_yes_predichos = c(""), VP = c(""), 
  VN = c(""), FP = c(""), FN = c("")))
...
Puesto que se considera más importante clasificar correctamente a aquellos pacientes que fallecen tras sufrir un infarto, **se propone cambiar el punto de corte a un valor de 0.2**.  


Por otra parte, vamos a realizar una tabla con parámetros de la logística:   

```{r eval = TRUE}
# Tabla de parámetros y signos de la regresión logística con variables seleccionadas del método MMPC:
...
Los signos dan cuenta de que la mayoría de las variables utilizadas, excepto "edad" o "creatinina", disminuyen la probabilidad de fallecer a lo largo del año siguiente a padecer un infarto.  


Como última opción, se procede a modelar un árbol de decisión simple con el objetivo de esclarecer el efecto de cada variable en el modelo. Este enfoque considera de manera conjunta tanto las interacciones como los efectos combinados de las demás variables presentes en el modelo.   


## 20: Árbol simple.

Este modelo no requiere de variables predictoras dummyficadas ni normalizadas. Con el fin de comprender mejor los resultados de nuestro árbol, las variables seleccionadas para el modelo son las precursoras de las variables seleccionadas mediante nuestro método MMPC elegido:   
...
En este árbol de decisión simple, se utilizan todas las variables, excepto "crepitantes". Además, se observa cómo estas variables se relacionan entre sí dentro del árbol.  

Existe una consonancia con los parámetros de la tabla logística: pacientes más mayores o con niveles de creatinina superiores a 1.3 mg/dL tienen mayor riesgo a fallecer, mientras que pacientes a los que se les haya suministrado inhibidores de la ECA o con niveles de sodio superiores a 137 mEq/L disminuyen su riesgo.   


## 21: Gráficos.

Para finalizar, se representan los siguientes gráficos con el objetivo de comprobar la capacidad de nuestro modelo y la separabilidad de nuestros datos:  
...
En relación a los gráficos obtenidos, cada paciente de nuestra base de datos es representado mediante un punto. En los cuatro primeros gráficos, los pacientes fallecidos se representan con un punto rojo, mientras que los pacientes no fallecidos se representan mediante un punto verde.   

El primer gráfico es un gráfico de puntos que representa la distribución de los pacientes en las dos primeras dimensiones del Análisis de Correspondencia Múltiple Factorial (FAMD en inglés). Además, se aprecian contornos de densidad que suponen un indicativo visual de la frecuencia de los puntos en el espacio FAMD.   

El segundo gráfico añade curvas de contorno que permiten visualizar la capacidad discriminatoria del algoritmo predictivo. Las curvas de contorno son diseñadas en función de las probabilidades estimadas por nuestro modelo elegido.   
 
En el tercer gráfico se añaden las variables seleccionadas mediante el método MMPC, empleadas para estimar las predicciones.   

El cuarto gráfico es una representación conjunta de los tres primeros gráficos.  

El quinto gráfico es de puntos, donde el color denota la probabilidad estimada por el modelo. Tonos rojos asignan una probabilidad alta de pertenencia a la case de pacientes fallecidos y tonos verdes implican una probabilidad baja de pertenencia a la misma clase.   

El sexto gráfico es un gráfico de puntos coloreado según la diferencia entre el valor de referencia de la variable objetivo y la probabilidad estimada por nuestro modelo.   

En general, se puede ver el desbalanceamiento de la variable objetivo, así como la mala separabilidad de los datos. Aun así, hay ciertas regiones donde existe una separación más clara de nuestras observaciones y que el modelo predictivo es capaz de discriminar correctamente.   


Para acabar, se detiene la paralelización:

```{r eval = TRUE}
# Se para la paralelización:
...
Este trabajo ha consistido en un estudio íntegro de un conjunto de datos; empezando por su análisis exploratorio, pasando por la construcción y comparación de diferentes modelos y finalizando con la elección del mejor modelo y comentando los resultados obtenidos del mismo.   

Con dichos resultados, se puede considerar adecuada nuestra elección del modelo Logística (MMPC). Este modelo se basa en un algoritmo de regresión logística, donde las variables predictoras han sido seleccionadas mediante el método MMPC. Los resultados pueden variar ligeramente al probar diferentes semillas de partición, razón por la cual se ha hecho especial hincapié en obtener un buen modelo al compararlo con muchos otros modelos.   

Las predicciones finales se han estimado a partir del 90% de las observaciones, siendo lo más fieles posible a la realidad (donde se construiría el modelo con el 100% de los datos conocidos y se aplicaría sobre nuevas observaciones).   

Por una parte, a través de los gráficos finales podríamos inferir una limitada eficiencia de las variables seleccionadas, ya que la separabilidad no es tan buena como esperaríamos a priori.  

Por otra parte, la elección de un nuevo punto de corte nos proporciona unos resultados más prometedores. En este caso, el algoritmo es capaz de detectar más verdaderos positivos (VP), aunque suponga un aumento del número de falsos positivos (FP) al mismo tiempo. Se prioriza la capacidad de detectar fallecidos correctamente, o, en su defecto, de clasificar a pacientes no fallecidos como pacientes con riesgo de fallecer para así ofrecerles un seguimiento más cercano y ser precavidos.   
...
En todo momento, se ha procurado seguir un razonamiento lógico combinando los resultados obtenidos con el sentido común. A su vez, se ha perseguido el entendimiento de todo proceso con el objetivo, entre otros, de poder aplicarlo a otros futuros proyectos. La implantación de otros posibles modelos alternativos tiene cabida en este trabajo, pudiendo complementar y aportar información adicional.   
